...
object KoskiToSuoritusConverter {
  ...
  def getYksilollistaminen(opiskeluoikeus: KoskiOpiskeluoikeus, suoritus: KoskiSuoritus): Option[PerusopetuksenYksilollistaminen] = {
    val yhteisetAineet = suoritus.osasuoritukset.getOrElse(Set.empty)
      .filter(os => os.koulutusmoduuli.flatMap(_.tunniste).map(_.koodiarvo).exists(YHTEISET_AINEET.contains))
  ...
  val YHTEISET_AINEET = YHTEISET_EI_KATSOMUSAINEET ++ YHTEISET_KATSOMUSAINEET
