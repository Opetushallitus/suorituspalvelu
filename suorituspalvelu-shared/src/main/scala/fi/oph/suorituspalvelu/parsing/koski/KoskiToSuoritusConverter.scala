...
object KoskiToSuoritusConverter {
  ...
  def toAmmatillisenTutkinnonOsa(osaSuoritus: KoskiOsaSuoritus): AmmatillisenTutkinnonOsa = {
    ...
    AmmatillisenTutkinnonOsa(
      ...
      laajuus = osaSuoritus.koulutusmoduuli.flatMap(k => k.laajuus.map(l => Laajuus(l.arvo, asKoodiObject(l.yksikkö.get), Option.apply(l.yksikkö.get.nimi), Option.apply(l.yksikkö.get.lyhytNimi.getOrElse(l.yksikkö.get.nimi))))),
      osaAlueet = osaSuoritus.osasuoritukset.map(osaSuoritukset => osaSuoritukset.map(osaSuoritus => toAmmattillisenTutkinnonOsaAlue(osaSuoritus))).getOrElse(Seq.empty),
      korotettu = osaSuoritus.korotettu.map(k => Korotus.valueOf(k.koodiarvo.toUpperCase))
    )
  }
  ...
  def toAmmatillinenPerustutkinto(opiskeluoikeus: KoskiOpiskeluoikeus, suoritus: KoskiSuoritus): AmmatillinenPerustutkinto =
    AmmatillinenPerustutkinto(
      ...
      suoritus.osasuoritukset.map(os => os.map(os => toAmmatillisenTutkinnonOsa(os))).getOrElse(Seq.empty)
    )
  ...
  def toVapaaSivistystyoKoulutus(opiskeluoikeus: KoskiOpiskeluoikeus, suoritus: KoskiSuoritus): VapaaSivistystyo =
    ...
    VapaaSivistystyo(
      ...
      suoritus.suorituskieli.map(k => asKoodiObject(k)).getOrElse(dummy()),
      parseLasnaolot(opiskeluoikeus, None, vahvistusPaivamaara).map(l =>
        Lahtokoulu(l._1, l._2, oppilaitos.oid, valmistumisVuosi, VAPAA_SIVISTYSTYO.defaultLuokka.get, supaTila, None, VAPAA_SIVISTYSTYO))
    )
  ...
  def toPerusopetuksenOppiaineet(osasuoritukset: Seq[KoskiOsaSuoritus], koodistoProvider: KoodistoProvider): Seq[PerusopetuksenOppiaine] = {
  ...
  def getYksilollistaminen(opiskeluoikeus: KoskiOpiskeluoikeus, suoritus: KoskiSuoritus): Option[PerusopetuksenYksilollistaminen] = {
    val yhteisetAineetJaKielet = suoritus.osasuoritukset.getOrElse(Seq.empty)
      .filter(os => os.koulutusmoduuli.exists(km => km.pakollinen.exists(p => p) || km.tunniste.map(_.koodiarvo).exists(KoskiUtil.sisallytettavatEiPakollisetKielet.contains)))
    ...
    (erityisiaAineita, opiskeleeToimintaAlueittain) match {
      ...
      case _ =>
        ...
        if (erityisiaAineita > yhteisetAineetJaKielet.size / 2) {
  ...
  // -Opiskeluoikeus, jolla on kotiopetusjakso jolla on loppupäivä, mutta kotiopetusjakso
  // ei ole loppunut ennen oppimäärän vahvistamista
  ...
  def getPerusopetuksenLahtokoulut(opiskeluoikeus: KoskiOpiskeluoikeus, haluttuLuokkaAste: String, supaTila: Option[SuoritusTila], yhteisenAineenArvosanaPuuttuu: Option[Boolean], seuraavanAsteenAlkamispaiva: Option[LocalDate], koodistoProvider: KoodistoProvider): List[Lahtokoulu] =
    ...
    opiskeluoikeus.suoritukset.get
      .filter(s => s.tyyppi.koodiarvo == SUORITUSTYYPPI_PERUSOPETUKSENVUOSILUOKKA).flatMap(s => {
        val tila = supaTila.getOrElse(parseTila(opiskeluoikeus, Some(s)).map(tila => convertKoskiTila(tila.koodiarvo)).getOrElse(dummy()))
        ...
        s.alkamispäivä match
          ...
          case Some(pvm) if luokkaAste==haluttuLuokkaAste =>
            ...
            Some(parseLasnaolot(opiskeluoikeus, Some(alkamispaiva), loppuPaivamaara).map(l => {
              Lahtokoulu(l._1, l._2, oppilaitos.oid, valmistumisVuosi, luokka, tila, yhteisenAineenArvosanaPuuttuu, LahtokouluTyyppi.valueOf(s"VUOSILUOKKA_$luokkaAste"))
            }))
          case default => None
      })
      .flatten
  ...
  def toPerusopetuksenOppimaara(opiskeluoikeus: KoskiOpiskeluoikeus, suoritus: KoskiSuoritus, koodistoProvider: KoodistoProvider): Option[PerusopetuksenOppimaara] = {
    ...
    if (kotiopetuslainen && !valmisErityinenTutkinto) {
      ...
      val lahtokoulut9 = getPerusopetuksenLahtokoulut(opiskeluoikeus, "9", Some(supaTila), Some(KoskiUtil.yhteisenAineenArvosanaPuuttuu(aineet)), None, koodistoProvider)
      ...
      val jaaLuokalle = opiskeluoikeus.suoritukset.getOrElse(Seq.empty)
        .filter(s => s.tyyppi.koodiarvo == SUORITUSTYYPPI_PERUSOPETUKSENVUOSILUOKKA)
  ...
  def toPerusopetukseenValmistavaOpetus(opiskeluoikeus: KoskiOpiskeluoikeus, suoritus: KoskiSuoritus): PerusopetukseenValmistavaOpetus = {
    ...
    PerusopetukseenValmistavaOpetus(lahtokoulut =
        parseLasnaolot(opiskeluoikeus, None, vahvistusPaivamaara).map(l =>
          Lahtokoulu(l._1, l._2, oppilaitos.oid, valmistumisVuosi, PERUSOPETUKSEEN_VALMISTAVA_OPETUS.defaultLuokka.get, supaTila.getOrElse(dummy()), None, PERUSOPETUKSEEN_VALMISTAVA_OPETUS))
    )
  }
  ...
  def toEbOppiaine(osaSuoritus: KoskiOsaSuoritus): EBOppiaine = {
    EBOppiaine(
      ...
      osasuoritukset = osaSuoritus.osasuoritukset.map((osaSuoritukset: Seq[KoskiOsaSuoritus]) => {
