...
case class NuortenPerusopetuksenOppiaineenOppimaara(nimi: String, koodi: String, arvosana: String) extends Suoritus, Tyypitetty

case class PerusopetuksenOppimaara(organisaatioOid: String, tila: Koodi, vahvistusPaivamaara: Option[LocalDate], aineet: Set[PerusopetuksenOppiaine]) extends Suoritus, Tyypitetty

case class PerusopetuksenOppiaine(nimi: String, koodi: String, arvosana: String) extends Tyypitetty

case class PerusopetuksenVuosiluokka(nimi: String, koodi: String, alkamisPaiva: Option[LocalDate]) extends Suoritus, Tyypitetty

case class PerusopetuksenOpiskeluoikeus(
                                         oid: String, oppilaitosOid: String,
                                         @JsonTypeInfo(use = JsonTypeInfo.Id.NAME, include = JsonTypeInfo.As.PROPERTY, property = "type")
                                         suoritukset: Seq[fi.oph.suorituspalvelu.business.Suoritus],
