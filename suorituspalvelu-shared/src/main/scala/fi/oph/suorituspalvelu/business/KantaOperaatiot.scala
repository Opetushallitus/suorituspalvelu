...
class KantaOperaatiot(db: JdbcBackend.JdbcDatabaseDef) {
   ...
   */
  def haeLahtokoulunOppilaat(paivamaara: LocalDate, oppilaitosOid: String, valmistumisVuosi: Int, luokka: Option[String], keskenTaiKeskeytynyt: Boolean, arvosanaPuuttuu: Boolean, lahtokouluTyypit: Set[LahtokouluTyyppi]): Set[(String, Option[String])] =
    Await.result(db.run(
      sql"""
          WITH
