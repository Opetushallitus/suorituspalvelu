...
*/
  ...
  def poistaVersionSuoritukset(versio: VersioEntiteetti): DBIOAction[_, NoStream, Effect] =
    DBIO.sequence(Seq(
      sqlu"""
          DELETE FROM perusopetuksen_oppimaarat USING versiot WHERE versiot.tunniste=perusopetuksen_oppimaarat.versio_tunniste;
       """,
      sqlu"""
          DELETE FROM ammatilliset_tutkinnot USING versiot WHERE versiot.tunniste=ammatilliset_tutkinnot.versio_tunniste;
       """
    ))
