    ...
    val arvot = (perusopetuksenOppimaara, hakemus) match {
      ...
      case (Some(po), _) if oppimaaraVahvistettuAjoissa(po, vahvistettuViimeistaan) =>
        ...
        val arvosanaJaKieliArvot = convertParhaatPeruskoulunArvosanatJaKielet(personOid, aineetPaasuoritukselta, aineetOppimaarilta).filter(aa => {
          ...
          aa.avain.endsWith(AvainArvoConstants.peruskouluAineenKieliTietoPostfix) ||
            aa.avain.endsWith(AvainArvoConstants.peruskouluAineenKieliOppiainePostfix) ||
            AvainArvoConstants.numeerisetPeruskoulunArvosanat.contains(aa.arvo)
        })
