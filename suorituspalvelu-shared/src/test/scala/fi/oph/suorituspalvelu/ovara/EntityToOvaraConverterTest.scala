...
class EntityToOvaraConverterTest {
  ...
  @Test def testGetGeneerisetOpiskeluoikeudet_LukioDIAEBIBTuvaVST(): Unit = {
    ...
    val ibOa = IBOppiaineSuoritus(UUID.randomUUID(), kielistetty("ibOa"), koodi("iboa"), Some(IBOppiaineRyhma(kielistetty("ryhma"), koodi("r1"))), Some(IBArvosana(koodi("6"), hyvaksytty = true)), Some(IBLaajuus(BigDecimal(150), koodi("h"))), Some(koodi("EN")), None, None)
