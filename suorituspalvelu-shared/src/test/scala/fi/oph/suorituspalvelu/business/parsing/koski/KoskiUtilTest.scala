...
class KoskiUtilTest {
  ...
  @Test def testHaeviimeisinLahtokoulu(): Unit =
    val opiskeluoikeus = PerusopetuksenOpiskeluoikeus(
      ...
      suoritukset = Set(
        PerusopetuksenOppimaara(
          ...
          aineet = Set.empty,
          lahtokoulut = Set(
            Lahtokoulu(LocalDate.parse("2024-08-18"), Some(LocalDate.parse("2025-08-18")), "ensimmäinen", Some(LocalDate.now.getYear), Some("9A"), Some(SuoritusTila.KESKEYTYNYT), None, VUOSILUOKKA_9),
            Lahtokoulu(LocalDate.parse("2024-01-01"), None, "toinen", Some(LocalDate.now.getYear), Some("9A"), Some(SuoritusTila.KESKEN), None, VUOSILUOKKA_9)
          ),
          syotetty = false
        ),
      ),
    ...
    Assertions.assertEquals(Some("toinen"), KoskiUtil.haeViimeisinLahtokoulu(LocalDate.parse("2025-02-20"), Set(opiskeluoikeus)))
