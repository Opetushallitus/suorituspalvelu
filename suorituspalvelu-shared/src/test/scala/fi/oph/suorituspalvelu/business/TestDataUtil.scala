...
import fi.oph.suorituspalvelu.business.{AmmatillinenOpiskeluoikeus, AmmatillinenPerustutkinto, AmmatillisenTutkinnonOsa, AmmatillisenTutkinnonOsaAlue, GeneerinenOpiskeluoikeus, Koodi, Laajuus, Lahtokoulu, LahtokouluTyyppi, OpiskeluoikeusJakso, Oppilaitos, Suoritus, SuoritusTila, Telma, Tuva, VapaaSivistystyo}
import fi.oph.suorituspalvelu.parsing.koski.{Kielistetty, KoskiOpiskeluoikeusTila, KoskiToSuoritusConverter}
...
object TestDataUtil {
  ...
  def getTestTuva(laajuusArvo: Option[BigDecimal] = Some(BigDecimal(10)),
    ...
    Tuva(
      ...
      Lahtokoulu(aloitusPaivamaara, vahvistusPaivamaara, oppilaitos.oid, Some(suoritusVuosi), "tuva", Some(supaTila), None, LahtokouluTyyppi.TUVA)
    )
  ...
  def getTestTelma(laajuusArvo: Option[BigDecimal] = Some(BigDecimal(13)),
    ...
    Telma(
      ...
      Lahtokoulu(aloitusPaivamaara, vahvistusPaivamaara, oppilaitos.oid, Some(suoritusVuosi), LahtokouluTyyppi.TELMA.defaultLuokka.get, Some(supaTila), None, LahtokouluTyyppi.TELMA)
    )
  ...
  def getTestVapaaSivistystyo(laajuusArvo: Option[BigDecimal] = Some(BigDecimal(14)),
    ...
    VapaaSivistystyo(
      ...
      laajuusArvo.map(arvo => Laajuus(arvo, Koodi("6", "opintojenlaajusyksikkö", Some(1)), None, None)),
      suoritusKieli,
      Lahtokoulu(aloitusPaivamaara, vahvistusPaivamaara, oppilaitos.oid, Some(suoritusVuosi), LahtokouluTyyppi.VAPAA_SIVISTYSTYO.defaultLuokka.get, Some(supaTila), None, LahtokouluTyyppi.VAPAA_SIVISTYSTYO)
    )
