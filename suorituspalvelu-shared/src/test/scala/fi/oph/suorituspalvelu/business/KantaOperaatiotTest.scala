...
class KantaOperaatiotTest {
  ...
  @Test def testGeneerinenOpiskeluoikeusEqualityAfterPersisting(): Unit =
    val OPPIJANUMERO1 = "1.2.246.562.24.99988877766"
    val versio = this.kantaOperaatiot.tallennaJarjestelmaVersio(OPPIJANUMERO1, KOSKI, "{\"attr\": \"value\"}").get
    val suoritus = Tuva(UUID.randomUUID(), Kielistetty(Some("Nimi Suomi"), None, None), Koodi("arvo", "koodisto", None), Oppilaitos(Kielistetty(Some("Nimi suomi"), None, None), "1.2.246.562.10.95136889433"), Koodi("lasna", "koskiopiskeluoikeudentila", Some(1)), Some(LocalDate.parse("2025-03-20")), Some(LocalDate.parse("2025-03-20")), None)
