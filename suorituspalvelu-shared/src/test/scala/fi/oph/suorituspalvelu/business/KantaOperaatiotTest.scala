...
class KantaOperaatiotTest {
  ...
  @Test def testHaeOhjattavatOppijat(): Unit =
    ...
    this.kantaOperaatiot.tallennaVersioonLiittyvatEntiteetit(versio.get, Set.empty, Seq(
      Lahtokoulu(LocalDate.now, None, oppilaitosOid, Some(valmistumisVuosi), None, Some(SuoritusTila.KESKEN), None, LahtokouluTyyppi.VUOSILUOKKA_9)
    ));

    Assertions.assertEquals(Set(henkiloNumero), this.kantaOperaatiot.haeLahtokoulunOppilaat(LocalDate.now, oppilaitosOid, valmistumisVuosi, None, false, false, Set(VUOSILUOKKA_9)))
  ...
  @Test def testHaeOhjattavatOppijatOppilaitosSiirto(): Unit =
    ...
    this.kantaOperaatiot.tallennaVersioonLiittyvatEntiteetit(versio.get, Set.empty, Seq(
      ...
      Lahtokoulu(LocalDate.now.minusDays(2), suorituksenLoppu = Some(LocalDate.now.minusDays(1)), vanhaOppilaitosOid, Some(valmistumisVuosi), None, Some(SuoritusTila.KESKEYTYNYT), None, LahtokouluTyyppi.VUOSILUOKKA_9),
      // jälkimmäinen suoritus alkaa kun ensimmäinen loppuu
      Lahtokoulu(LocalDate.now.minusDays(1), None, uusiOppilaitosOid, Some(valmistumisVuosi), None, Some(SuoritusTila.KESKEN), None, LahtokouluTyyppi.VUOSILUOKKA_9)
    ))
    ...
    Assertions.assertEquals(Set(henkiloNumero), this.kantaOperaatiot.haeLahtokoulunOppilaat(LocalDate.now, vanhaOppilaitosOid, valmistumisVuosi, None, false, false, Set(VUOSILUOKKA_9)))
    // ja luonnollisesti näkyy myös uuden koulun listalla
    Assertions.assertEquals(Set(henkiloNumero), this.kantaOperaatiot.haeLahtokoulunOppilaat(LocalDate.now, uusiOppilaitosOid, valmistumisVuosi, None, false, false, Set(VUOSILUOKKA_9)))
