...
import fi.oph.suorituspalvelu.mankeli.{AvainArvoSovitin, AvainArvoConstants}
...
class AvainArvoSovitinTest {
  ...
  @Test def testKoskiParsingForAmmatillistenOpiskeluoikeuksienSuoritukset(): Unit = {
    ...
    splitData.foreach((oppijaOid, data) => {
      ...
      val foo = AvainArvoSovitin.mankeloiPeruskouluarvot("1.2.246.562.98.69863082363", oos)

      Assertions.assertEquals(Some("9"), foo.keyValues.get(AvainArvoConstants.peruskouluAineenArvosanaPrefixes.head+"GE"))
      Assertions.assertEquals(Some("FI"), foo.keyValues.get(AvainArvoConstants.perusopetuksenKieliKeys.head))
      Assertions.assertEquals(Some("2025"), foo.keyValues.get(AvainArvoConstants.peruskouluSuoritusvuosiKeys.head))
      Assertions.assertEquals(Some("true"), foo.keyValues.get(AvainArvoConstants.peruskouluSuoritettuKeys.head))
    })
  }
