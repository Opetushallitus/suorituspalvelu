...
class LegacySuorituksetIntegraatioTest extends BaseIntegraatioTesti {
  ...
  @Test def testLegacySuorituksetBothParameters(): Unit =
    // vain jompi kumpi parametreista saa olla määritelty
    val result = mvc.perform(MockMvcRequestBuilders.get(ApiConstants.LEGACY_SUORITUKSET_PATH + "?" +
  ...
  @Test def testLegacySuorituksetAllowed(): Unit =
    ...
    val ammatillinenTutkinto = AmmatillinenTutkinto("diplomi", Koodi("123456", "koulutus", 1), Koodi("valmistunut", "jokutila", 1), Some(LocalDate.now()), None, Koodi("tapa", "suoritustapa", 1), Set.empty)
    ...
    Assertions.assertEquals(Seq(ammatillinenTutkinto).map(t => LegacySuoritus(OPPIJA_OID, "ammatillinentutkinto", "123456", "VALMIS")), legacySuorituksetResponse)
