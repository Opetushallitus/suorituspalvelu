...
käytännössä tarvitaan henkilöoid: automatic eligibility job käyttää tätä niiden oppiojiden tunnistamiseen joilla muuttuneita suorituksia, tähän tarvitsee kutsuun myös lastmodified -parametrin
lisäksi tarvitaan tila, koska atarusta tulee kutsu jolla selvitetään onko oppijalla ylioppilas- tai ammatillinen tutkinto, tähän tarvitaan komo-parametri

                           @(Schema @field)(example = "koski") @BeanProperty source: String,
                           @(Schema @field)(example = "true") @BeanProperty vahvistettu: Boolean,
                           @(Schema @field)(example = "1.2.246.562.13.62959769647") @BeanProperty komo: String,
                           @(Schema @field)(example = "1.2.246.562.10.42923230215") @BeanProperty myontaja: String,
                           @(Schema @field)(example = "16.04.2022") @BeanProperty valmistuminen: String,
                           @(Schema @field)(example = "Ei") @BeanProperty yksilollistaminen: String,
                           @(Schema @field)(example = "FI") @BeanProperty suoritusKieli: String,
                           @(Schema @field)(example = "01706b39-09a5-461f-bbf9-431196b9b502") @BeanProperty id: String,
                           @(Schema @field)(example = "{ \"last modified\": \"1747477417224\" }") @BeanProperty lahdeArvot: Map[String, String]
  ...
  def legacySuoritukset(
    ...
    try
        ...
        Right(None)
          ...
          .map(_ =>
            ...
            if(oppijaNumero.isPresent)
              ...
              AuditLog.logCreate(user, Map(LEGACY_SUORITUKSET_MUOKATTU_JALKEEN_PARAM_NAME -> muokattuJalkeen.orElse(null)), AuditOperation.HaeYoTaiAmmatillinenTutkintoTiedot, null)
      ...
      case e: Exception =>
        ...
        ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(VirtaSyncFailureResponse(java.util.List.of(YO_TAI_AMMATILLISTEN_HAKU_EPAONNISTUI)))
