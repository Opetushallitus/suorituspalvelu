...
class UIService {
  ...
  def haeOhjattavat(oppilaitos: String, valmistumisVuosi: Int, luokka: Option[String], keskenTaiKeskeytynyt: Boolean, yhteistenArvosanaPuuttuu: Boolean): Seq[Oppija] = {
    ...
    val oppijaOids = kantaOperaatiot.haeLahtokoulunOppilaat(LocalDate.now, oppilaitos, valmistumisVuosi, luokka, keskenTaiKeskeytynyt, yhteistenArvosanaPuuttuu, Set(VUOSILUOKKA_9, TELMA, TUVA, VAPAA_SIVISTYSTYO))
