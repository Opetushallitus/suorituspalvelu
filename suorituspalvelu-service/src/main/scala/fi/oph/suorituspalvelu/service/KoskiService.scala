  ...
  def refreshKoskiChangesSince(ctx: SupaJobContext, since: Instant): SaferIterator[SyncResultForHenkilo] =
    ...
    tiedot.grouped(100).flatMap(chunk => {
      ...
      def isYsiluokkalainenTaiLisapiste(koskiData: KoskiDataForOppija): Boolean =
        ...
        KoskiUtil.onkoJokinLahtokoulu(LocalDate.now, None, Some(Set(VUOSILUOKKA_9, TUVA, TELMA, VAPAA_SIVISTYSTYO)), opiskeluoikeudet.toSet) ||
        KoskiUtil.onkoJokinLahtokoulu(LocalDate.now, None, Some(Set(VUOSILUOKKA_9, TUVA, TELMA, VAPAA_SIVISTYSTYO)), kantaOperaatiot.haeSuoritukset(koskiData.oppijaOid).values.flatten.toSet)
