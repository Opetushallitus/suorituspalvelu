...
import fi.oph.suorituspalvelu.resource.ui.{SyotettyPerusopetuksenOppimaaranSuoritus, SyotettyPerusopetuksenOppiaineenOppimaarienSuoritusContainer}
  ...
  private val reparseKoskiJob = scheduler.registerJob("reparse-koski-data", (ctx, dryRun) => {
    ...
    versiot.zipWithIndex.foreach((versio, idx) => {
        try
          ...
          val (_, data) = kantaOperaatiot.haeData(versio)
          ...
          // TODO: lisää metadata kun OPHSUPA-142 mergattu
          if(!dryRun.toBoolean) kantaOperaatiot.tallennaVersioonLiittyvatEntiteetit(versio, converted, Map.empty)
        catch
