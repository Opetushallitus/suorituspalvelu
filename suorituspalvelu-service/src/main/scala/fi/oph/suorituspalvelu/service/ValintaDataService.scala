...
case class ValintaData(personOid: String,
                       paatellytAvainArvot: Seq[CombinedAvainArvoContainer],
                       avainArvoMetadatat: Seq[AvainMetatiedotDTO],
                       hakemus: Option[ConvertedAtaruHakemus],
...
class ValintaDataService {
  ...
  def doAvainArvoConversions(personOid: Option[String], haku: KoutaHaku, hakemus: Option[AtaruValintalaskentaHakemus], allOidsForPerson: Set[String]): ValintaData = {
    ...
    val rawResults = AvainArvoConverter.convertOpiskeluoikeudet(usePersonOid, vahvistettuViimeistaan, hakemus, kaikkiOpiskeluoikeudet, opiskeluoikeudetVahvistettuHetkella, haku, harkinnanvaraisuudet, hakukohderyhmat)
    ...
    ValintaData(usePersonOid, combinedWithYliajot.toSeq, yoMetadata, rawResults.convertedHakemus, kaikkiOpiskeluoikeudet, vahvistettuViimeistaan, suoritustenAjanhetki, harkinnanvaraisuudet, ensikertalaisuusTulos)
  }
  ...
  def getValintaData(personOid: String, hakuOid: String): ValintaData = {
    ...
    val isToisenAsteenHaku = haku.isToisenAsteenHaku()
    val resultF = for {
