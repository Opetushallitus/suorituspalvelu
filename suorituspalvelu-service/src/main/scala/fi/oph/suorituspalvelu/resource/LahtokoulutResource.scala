...
class LahtokoulutResource {
  ...
  @GetMapping(
    path = Array(LAHTOKOULU_OPISKELIJAT_PATH),
    produces = Array(MediaType.APPLICATION_JSON_VALUE)
  )
  @Operation(
  ...
  def haeLahtokoulu(@PathVariable(name = LAHTOKOULU_HENKILOOID_PARAM_NAME, required = false) @Parameter(description = "Henkilön tunniste", example = ESIMERKKI_OPPIJANUMERO) henkiloOid: Optional[String],
                    request: HttpServletRequest): ResponseEntity[LahtokouluResponse] = {
    try
      val securityOperaatiot = new SecurityOperaatiot
      LogContext(path = LAHTOKOULU_OPISKELIJAT_PATH, identiteetti = securityOperaatiot.getIdentiteetti())(() =>
        Right(None)
          .flatMap(_ =>
            // tarkastetaan oikeudet
            if (securityOperaatiot.onRekisterinpitaja() || securityOperaatiot.onPalveluKayttaja())
          ...
          .map(henkiloOid => {
            try
              val user = AuditLog.getUser(request)
              AuditLog.log(user, Map(LAHTOKOULU_HENKILOOID_PARAM_NAME -> henkiloOid), AuditOperation.HaeLahtokoulut, None)
