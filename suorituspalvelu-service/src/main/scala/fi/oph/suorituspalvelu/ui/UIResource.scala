...
class UIResource {
      ...
      LogContext(path = UI_TALLENNA_SUORITUS_PERUSOPETUS_PATH, identiteetti = securityOperaatiot.getIdentiteetti())(() =>
        Right(None)
          ...
          .flatMap(suoritus =>
            ...
            val now = Instant.now()
            val versio = this.kantaOperaatiot.tallennaJarjestelmaVersio(suoritus.oppijaOid.get(), SuoritusJoukko.SYOTETTY_PERUSOPETUS, Seq(objectMapper.writeValueAsString(suoritus)), Seq.empty, now, SuoritusJoukko.defaultLahdeTunniste(SuoritusJoukko.SYOTETTY_PERUSOPETUS), None)
  ...
  def poistaOppijanYliajo(
                           ...
                           @RequestParam(name = UI_VALINTADATA_AVAIN_PARAM_NAME, required = false) @Parameter(description = "poiston selite", example = ESIMERKKI_YLIAJO_SELITE) selite: Optional[String],
