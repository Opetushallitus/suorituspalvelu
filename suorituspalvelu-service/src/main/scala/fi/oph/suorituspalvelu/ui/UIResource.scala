    ...
    try {
      ...
      LogContext(path = UI_TIEDOT_PATH, identiteetti = securityOperaatiot.getIdentiteetti())(() =>
        Right(None)
          ...
          .flatMap(_ =>
            val virheAvaimet = UIValidator.validateOppijanumero(oppijaNumero.toScala, pakollinen = true)
