...
class UIResource {
  ...
  @Operation(
    ...
    requestBody =
      new io.swagger.v3.oas.annotations.parameters.RequestBody(
        content = Array(new Content(schema = new Schema(implementation = classOf[SyotettyPerusopetuksenOppiaineenOppimaaranSuoritusContainer])))),
    responses = Array(
      ...
      LogContext(path = UI_TALLENNA_SUORITUS_OPPIAINE_PATH, identiteetti = securityOperaatiot.getIdentiteetti())(() =>
        Right(None)
          ...
          .flatMap(_ =>
            // deserialisoidaan
            try
              Right(objectMapper.readValue(suoritusBytes, classOf[SyotettyPerusopetuksenOppiaineenOppimaaranSuoritusContainer]))
            catch
              case e: Exception =>
