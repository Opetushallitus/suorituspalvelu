...
class UIResource {
  ...
  @Operation(
    summary = "Palauttaa oppijan haut käyttöliittymälle",
    ...
    responses = Array(
      new ApiResponse(responseCode = "200", description = "Sisältää oppijan hauista lista Haku-objekteja (hakuOid ja nimi)", content = Array(new Content(schema = new Schema(implementation = classOf[OppijanHautSuccessResponse])))),
  ...
  def haeOppijaHaut(@PathVariable(UI_OPPIJAN_HAUT_OPPIJANUMERO_PARAM_NAME) @Parameter(description = "Oppijanumero", example = ESIMERKKI_OPPIJANUMERO, required = true) oppijaNumero: Optional[String],
    ...
    try
      ...
      LogContext(path = UI_OPPIJAN_HAUT_PATH, identiteetti = securityOperaatiot.getIdentiteetti())(() =>
        Right(None)
          ...
          .flatMap(oppijaNumero =>
            ...
            val user = AuditLog.getUser(request)
            AuditLog.log(user, Map(UI_OPPIJAN_HAUT_OPPIJANUMERO_PARAM_NAME -> oppijaNumero), AuditOperation.HaeOppijaTiedotUI, None)
      ...
      case e: Exception =>
        LOG.error("Oppijan hautien haku käyttöliitymälle epäonnistui", e)
