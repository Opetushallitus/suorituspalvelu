...
/*
    PerusopetuksenVuosiluokka(
      ...
      Some(LocalDate.parse(s"${aloitusVuosi}-08-01")),
      if (valmisVuosi.isDefined) Some(LocalDate.parse(s"${valmisVuosi.get}-06-01")) else None,
      false
    )
*/
...
/*
    tallennaOppimaara(OPPIJANUMERO_YSI_VALMIS_TAMA_VUOSI, VALMIS,
      Set(
        getYsiLuokka(VIIMEVUOSI, Some(TAMA_VUOSI)),
        getOppimaara(Some(TAMA_VUOSI))
      )
    )

    tallennaOppimaara(OPPIJANUMERO_YSI_VALMIS_VIIMEVUOSI, VALMIS,
      Set(
        getYsiLuokka(TOISSAVUOSI, Some(VIIMEVUOSI)),
