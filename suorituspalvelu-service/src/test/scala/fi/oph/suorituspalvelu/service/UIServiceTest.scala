  ...
  @Test def testCheckAccessOrganisaatioPermission(): Unit =
    ...
    val opiskeluoikeudet: Set[Opiskeluoikeus] = Set(PerusopetuksenOpiskeluoikeus(
      ...
      Set(PerusopetuksenOppimaara(
        ...
        Seq.empty,
        List(Lahtokoulu(LocalDate.parse(s"${valmistumisVuosi-1}-08-01"), Some(LocalDate.parse(s"$valmistumisVuosi-06-01")), oppilaitosOid, Some(LocalDate.now.getYear), "9A", VALMIS, None, VUOSILUOKKA_9)),
        false,
        false,
        None
      )),
  ...
  @Test def testCheckAccessOrganisaatioNoPermission(): Unit =
    ...
    val opiskeluoikeudet: Set[Opiskeluoikeus] = Set(PerusopetuksenOpiskeluoikeus(
      ...
      Set(PerusopetuksenOppimaara(
        ...
        Some(LocalDate.parse(s"$valmistumisVuosi-06-01")),
        Seq.empty,
        List(Lahtokoulu(LocalDate.parse(s"${valmistumisVuosi-1}-08-01"), Some(LocalDate.parse(s"$valmistumisVuosi-06-01")), oppilaitosJossaSuoritusOid, Some(LocalDate.now.getYear), "9A", VALMIS, None, VUOSILUOKKA_9)),
        false,
        false,
        None
      )),
