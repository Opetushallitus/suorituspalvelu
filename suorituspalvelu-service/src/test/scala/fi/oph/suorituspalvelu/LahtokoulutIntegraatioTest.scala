...
class LahtokoulutIntegraatioTest extends BaseIntegraatioTesti {
  ...
  @Test def testHaeLahtokoulutAnonymous(): Unit =
    // tuntematon käyttäjä ohjataan tunnistautumiseen
    mvc.perform(MockMvcRequestBuilders
        .get(ApiConstants.LAHTOKOULU_OPISKELIJAT_PATH
          .replace(ApiConstants.LAHTOKOULU_HENKILOOID_PARAM_PLACEHOLDER, ApiConstants.ESIMERKKI_OPPIJANUMERO), ""))
      .andExpect(status().is3xxRedirection())
  ...
  @Test def testHaeLahtokoulutInvalidParams(): Unit =
    ...
    val result = mvc.perform(MockMvcRequestBuilders
        .get(ApiConstants.LAHTOKOULU_OPISKELIJAT_PATH
          .replace(ApiConstants.LAHTOKOULU_HENKILOOID_PARAM_PLACEHOLDER, "Tämä ei ole validi oid"), ""))
      .andExpect(status().isBadRequest).andReturn()
  ...
  @Test def testHaeLahtokoulutAllowed(): Unit =
    ...
    val result = mvc.perform(MockMvcRequestBuilders
        .get(ApiConstants.LAHTOKOULU_OPISKELIJAT_PATH
          .replace(ApiConstants.LAHTOKOULU_HENKILOOID_PARAM_PLACEHOLDER, oppijaNumero), ""))
      .andExpect(status().isOk).andReturn()

    // henkilön luokka vastaa (toistaiseksi hardkoodattua) luokkaa
    Assertions.assertEquals(LahtokouluSuccessResponse(List(fi.oph.suorituspalvelu.resource.api.Lahtokoulu(OPPILAITOS_OID, aloitusPaiva, Optional.of(valmistumisPaiva), Optional.of("9A"), VUOSILUOKKA_9.toString)).asJava),
    ...
    Assertions.assertEquals(Map(
      ApiConstants.LAHTOKOULU_HENKILOOID_PARAM_NAME -> oppijaNumero,
    ), auditLogEntry.target)
}
